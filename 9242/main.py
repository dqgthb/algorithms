...
def main(f=None):
    ...
    # ######## INPUT AREA BEGIN ##########
    global mat
    mat = [[' ' for _ in range(32)] for _ in range(5)]

    maxLen = 0
    for i in range(5):
        for j, e in enu(input()):
            mat[i][j] = e
    ...
    codes = []
    for i in range(0, numCode * 4, 4):
        code = extract(i)
        codes.append(code)

    codeStrings = []
    for code in codes:
        flat = [''.join(i) for i in code]
        codeStrings.append(''.join(flat))
    ...
    for k in range(10):
        digitString = []
        for i in range(5):
            for j in range(k * 4, k * 4 + 3):
                digitString.append(digits[i][j])

        digitStrings[''.join(digitString)] = str(k)

    parsed = []
    for code in codeStrings:
        if code in digitStrings:
            parsed.append(digitStrings[code])
        else:
            print("BOOM!!")
            return
    parsedInt = int(''.join(parsed))
    print(parsedInt)
